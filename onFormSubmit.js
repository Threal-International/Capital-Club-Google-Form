...
function updateStatus(e) {
  ...
  var limitSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(membersSheet);
  ...
  for (var i = 1; i < limitData.length; i++) {
    ...
    if (
      limitData[i][2].toString() == userId &&
      limitData[i][1].toString() == userEmail
    ) {
      isMember = true;
      break;
    }
  ...
  var sheetID =
    PropertiesService.getScriptProperties().getProperty("FORM_SHEET_ID");
  if (!sheetID) {
  ...
  for (var i = 1; i < responseData.length; i++) {
    if (responseData[i][idColumn].toString() == userId) {
      if (
        responseData[i][statusColumn] == "Accepted" &&
        responseData[i][repaymentColumn] != "Paid"
      ) {
  ...
  if (!hasOutstandingLoan) {
    ...
    if (appliedAmount <= userLimit) {
      ...
      sheet
        .getRange(row, 9)
        .setValue("Requested amount is above your borrowing limit");
    }
  } else {
