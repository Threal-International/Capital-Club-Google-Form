function sendEmail() {
  var sheetGID = PropertiesService.getScriptProperties().getProperty("FORM_SHEET_GID");
  if (!sheetGID) {
  ...
  var treasEmail = PropertiesService.getScriptProperties().getProperty("TREASURER_EMAIL");
  if (!treasEmail) {
  ...
  var htmlForEmail = htmlTemplate.evaluate().getContent();
  GmailApp.sendEmail(treasEmail, "Nofitication for a loan application", "This email contains html", { htmlBody: htmlForEmail })
}
